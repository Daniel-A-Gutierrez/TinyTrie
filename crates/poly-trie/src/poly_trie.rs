...
impl NodeRef {
    ...
    pub fn is_internal(&self) -> bool {
        matches!(self, NodeRef::Node2 { .. } | NodeRef::Node4 { .. } | NodeRef::Node16 { .. })
    }
    ...
    pub fn get(&self, key: &[u8]) -> Option<usize> {
        ...
        loop {
            match current {
                ...
                NodeRef::Leaf { idx, .. } => {
                    let ki = idx as usize;
                    return if self.keys[ki] == key { Some(ki) } else { None };
                }
                _ => {
                ...
                _ => {
                    ...
                    match simd_find_divergence::<8>(new_key, ref_key, confirmed) {
                        ...
                        DivergeResult::At(diverge) if diverge < prefix_len => {
                            ...
                            parent_children[new_bit] =
                                NodeRef::leaf(diverge as u16, new_index as u32);
                        ...
                        DivergeResult::At(_) => {
                            ...
                            if matches!(child, NodeRef::Empty) {
                                ...
                                children[digit] =
                                    NodeRef::leaf(prefix_len as u16, new_index as u32);
            ...
            for (parent_digit, child) in children.iter().enumerate() {
                ...
                match *child {
                    ...
                    NodeRef::Leaf { idx: key_idx, .. } => {
                        ...
                        let wide_digit =
                            digit_at(key, prefix_len, new_radix_bits as u32) as usize;
                        debug_assert!(
                            wide_digit >= base && wide_digit < base + factor,
                        ...
                        debug_assert!(
                            matches!(new_children[wide_digit], NodeRef::Empty),
                            "collision in aligned merging (should be impossible)"
                        );
                        new_children[wide_digit] = NodeRef::leaf(
                            (prefix_len + new_radix_bits as usize) as u16,
                            key_idx,
                        );
                    }
                    _ => {
                        ...
                        for (child_digit, grandchild) in gc.iter().enumerate() {
                            if !matches!(grandchild, NodeRef::Empty) {
                                ...
                                debug_assert!(
                                    matches!(new_children[target], NodeRef::Empty),
                                    "collision in aligned merging (should be impossible)"
                                );
        ...
        fn dump_ref<T>(trie: &PolyTrie<T>, node_ref: NodeRef, depth: usize) {
            ...
            match node_ref {
                ...
                NodeRef::Leaf { prefix_len, idx } => {
                    ...
                    eprintln!(
                        "{indent}LEAF pfx={prefix_len} key_idx={idx} key={:?}",
...
fn mask_next(mask: u16, start: usize) -> Option<usize> {
    ...
    if shifted != 0 { Some(start + shifted.trailing_zeros() as usize) } else { None }
}
...
fn mask_prev(mask: u16, end: usize) -> Option<usize> {
    ...
    if below != 0 { Some(15 - below.leading_zeros() as usize) } else { None }
}
// ---------------------------------------------------------------------------
// PolyIter
// ---------------------------------------------------------------------------
...
impl<'a, T> PolyIter<'a, T> {
    ...
    fn new(trie: &'a PolyTrie<T>) -> Self {
        match trie.root {
            ...
            NodeRef::Leaf { .. } => {
                ...
                PolyIter {
                    trie,
                    stack: vec![Frame { node: NodeRef::Empty, slot: usize::MAX, mask: 0 }],
                }
            }
            _ => {
                ...
                PolyIter {
                    trie,
                    stack: vec![Frame { node: trie.root, slot: usize::MAX, mask }],
                }
            }
        }
    }
    ...
    fn descend_first(&mut self, mut node: NodeRef) {
        loop {
            ...
            let mask = compute_mask(children);
            let first_slot =
                mask_next(mask, 0).expect("internal node must have at least one child");
    ...
    fn descend_last(&mut self, mut node: NodeRef) {
        loop {
            ...
            let mask = compute_mask(children);
            let last_slot =
                mask_prev(mask, width).expect("internal node must have at least one child");
    ...
    pub fn seek(&mut self, key: &[u8]) -> Option<(&[u8], &T)> {
        ...
        loop {
            ...
            if !matches!(children[digit], NodeRef::Empty) {
                ...
                if let NodeRef::Leaf { idx: key_idx, .. } = child {
                    ...
                    if let Some(slot) = self
                        .find_next_slot(&Frame { node: current, slot: digit, mask }, digit + 1)
                    {
                    ...
                    loop {
                        ...
                        let parent_next =
                            self.find_next_slot(&parent_frame, parent_frame.slot + 1);
                        if let Some(slot) = parent_next {
                            self.stack.push(Frame {
                                node: parent_frame.node,
                                slot,
                                mask: parent_frame.mask,
                            });
                            let parent_width = parent_frame.node.width();
                            let parent_children = self
                                .trie
                                .arena
                                .get_range(parent_frame.node.idx(), parent_width);
            ...
            if let Some(slot) =
                self.find_next_slot(&Frame { node: current, slot: digit, mask }, digit + 1)
            {
                self.stack.push(Frame { node: current, slot, mask });
                let next_child = children[slot];
                if matches!(next_child, NodeRef::Leaf { .. }) {
            ...
            loop {
                let parent_frame = self.stack.pop()?;
                debug_assert!(
                    !matches!(parent_frame.node, NodeRef::Empty),
                ...
                if let Some(slot) = parent_next {
                    ...
                    let parent_width = parent_frame.node.width();
                    let parent_children =
                        self.trie.arena.get_range(parent_frame.node.idx(), parent_width);
                    let next_child = parent_children[slot];
                    if matches!(next_child, NodeRef::Leaf { .. }) {
                        return self.current();
                    } else {
