...
// use doa::block_cursor::{BlockCursor, Cursor, CursorMut};
// use doa::block::{BlockBase, BlockBaseMut, OpenSlot, SparseBlock, TreeBlock, Uniform, UniformBlock};
// use doa::index::BlockIndex;
// use doa::node::{D, HasParent, INode, LNode, Node, OrphanUnionNode, SplittableNode, UnionNode};
// use doa::store::{NoneSlide, VecStore};
// use doa::walker::*;
// use doa::PreOrder;
// use arrays::tiny_array::TinyArray;
// //node-agnostic constructors: build a cursor from a block borrow + meta. they know nothing
// //of the concrete node type, so they live here on `Probe`, generic over `B`/`M`. the walker
// //is constructed from a `TreeBlock<O>` ref — the `O` must match (the block is O-agnostic
// //storage; per-O behavior lives in the walker impls). root is read from `meta().root`.
// impl<'block, 'walker, P, B, M> Probe<'block, B, BlockCursor<'block, 'walker, B, &'walker B>, M>
// where
//     'block: 'walker,
//     P: BlockIndex,
//     B: UniformBlock<'block> + BlockBase<'block, P = P, Meta = BTreeMeta<P>>,
//     B::T: Node,
// {
//     pub fn new(meta: M, block: &'walker B) -> Self {
//         Self {
//             cursor: BlockCursor::new_at(block, block.meta().root),
//             depth: 0,
//             meta,
//             _l: PhantomData,
//         }
...
// impl<'block, 'walker, P, B, M> Probe<'block, B, BlockCursor<'block, 'walker, B, &'walker mut B>, M>
// where
//     'block: 'walker,
//     P: BlockIndex,
//     B: UniformBlock<'block> + BlockBase<'block, P = P, Meta = BTreeMeta<P>>,
...
// impl<'block, 'walker, P, B, M> Walker<'block, B, BlockCursor<'block, 'walker, B, &'walker mut B>, M>
// where
//     'block: 'walker,
//     P: BlockIndex,
//     B: UniformBlock<'block> + BlockBase<'block, P = P, Meta = BTreeMeta<P>>,
...
//     fn fixup_stack(&mut self, _ns: NoneSlide) {
//         todo!();
//     }
// }

// impl<'block, 'walker, P, B, M> Walker<'block, B, BlockCursor<'block, 'walker, B, &'walker B>, M>
// where
//     'block: 'walker,
//     P: BlockIndex,
//     B: UniformBlock<'block> + BlockBase<'block, P = P, Meta = BTreeMeta<P>>,
...
//     P: BlockIndex,
// {
//     fn keys(&self) -> impl doa::node::DoubleExact<Item = &Self::K> {
...
//     fn children(&self) -> impl doa::node::DoubleExact<Item = &P> {
...
//     P: BlockIndex,
// {
//     fn values(&self) -> impl doa::node::DoubleExact<Item = &V> {
//         return self.values.as_slice().iter();
//     }

//     fn pairs(&self) -> impl doa::node::DoubleExact<Item = (&K, &V)> {
//         return self.keys().zip(self.values());
//     }

//     fn keys(&self) -> impl doa::node::DoubleExact<Item = &K> {
...
// //`UnionNode` is foreign, so a `Default` impl would be orphan; a free ctor stands in.
// fn bnode_leaf<K: C, V: C, P: BlockIndex>() -> BNode<K, V, P> {
//     BNode { orphan: OrphanUnionNode { lnode: BLNode::default() }, parent: P::MIN }
// }
...
// where
//     'block: 'walker,
//     B: TreeBlock<'block, PreOrder> + BlockBase<'block, T = BNode<K, V, P>, P = P, Meta = BTreeMeta<P>>,
//     K: C + Ord,
//     V: C,
//     P: BlockIndex,
...
//     fn swap_none(&mut self, other: doa::block::OpenSlot) {
...
//         let root_v = self.cursor.meta().root;
...
//         block.set_meta(BTreeMeta { height: block.meta().height, root: root_v });
...
//             let found = rc.find_slot(first, false);
...
//             (rc.p2v(phys), phys)
//         };
//         right.set_meta(BTreeMeta { height: right.meta().height, root: new_root_v });
...
//         let found = self.cursor.find_slot(anchor_phys, true);
...
//         self.fixup(&ns);
//         let opened = self.cursor.slide_none(ns);
...
//         let (new_r_v, _) = self.place_at(leaf_phys, parent_v, bnode_leaf());
//         //L (anchor) may have moved; re-route k from the root to L's current vaddr.
//         self.cursor.vseek(self.cursor.meta().root);
//         self.depth = 0;
//         self.walk_to(&k);
...
//         debug_assert_eq!(self.cursor.v2p(self.cursor.meta().root), l_phys, "split_root: cursor not on root");
//         //place an empty inode R after L (find_slot grows + opens a slot on the after side;
//         //pin = L keeps L put). block: [L(0)] -> [L(0), R(1)].
//         let found = self.cursor.find_slot(l_phys, true);
//         let opened = found
//             .slide
//             .map(|ns| self.cursor.slide_none(ns))
...
//         self.cursor.set_meta(BTreeMeta { height: self.cursor.meta().height, root: root_v });
...
//         self.cursor.set_meta(BTreeMeta { height: Height(self.meta.0 + 1), root: self.cursor.meta().root });
...
//         debug_assert_eq!(self.cursor.v2p(self.cursor.meta().root), y_phys, "split_root_internal: not on root");
//         let found = self.cursor.find_slot(y_phys, true);
//         let opened = found
//             .slide
//             .map(|ns| self.cursor.slide_none(ns))
...
//     doa::block::Block<'static, BNode<K, V, P>, P, VecStore<BNode<K, V, P>, CAP>, Uniform, BTreeMeta<P>>;
...
//         let mut block = <MapBlock<K, V, P, CAP> as UniformBlock>::new();
//         let phys = block.insert_root(bnode_leaf());
//         block.set_meta(BTreeMeta { height: Height::default(), root: block.p2v(phys) });
//         Self { block, len: 0 }
//     }
...
//         let mut w = Probe::new(self.block.meta().height, &self.block);
//         w.walk_to(k)?;
...
//         let height = self.block.meta().height;
