...
// performs a phys/address mutation and reports the ptr transform it applied as
// an ArenaInsertDelta. ArenaInsertDelta is the union of these return types,
// derived bottom-up. It is ARENA-INTERNAL: `execute` consumes it to drive the
// walker (apply_relocs rewrites in-arena ptrs in place via unsafe disjoint
// borrows) on the walker/revptr paths — no RemapSet returned to the consumer —
// or to build the consumer-facing RemapSet on the pointerless path. The consumer
// never sees ArenaInsertDelta, Transform, or phys.
//
// These run AUTOMATICALLY at the arena tier — transparent to the consumer,
// which has no knowledge of any rebalancing that happened during place. Block-
// direct users (no arena) run these themselves; the arena can't and doesn't.
//
...
// Transform — closed-form remap, one variant per shift direction.
...
// Reloc — a partition piece: ptrs whose old addr (in old_block) fell in
// `old_range` now live at (new_block, transform.apply(old)). (Named Chunk in an
// earlier draft; node_traits.md calls this Reloc — same thing.) old_block is the
// block the op ran on (implicit for the single-block ops below, filled in when
// lifted to arena-level Reloc). Arena-internal; condensed to RemapSet only on
// the pointerless path.
struct Reloc { old_block: usize, old_range: (isize, isize), new_block: usize, transform: Transform }
...
//   vptr: new = old * 2  -> Transform::Left{1,0}. Same block.
...
//   v_offset += delta.
//   vptr: new = old - delta  -> Transform::Left{0,-delta}. Same block.
//   Returns Readdressed.
...
//   vptr: unchanged in both halves  -> Transform::Left{0,0}. Halving density opens
//   gaps (fixes OutOfBudget); addresses kept, so doesn't fix end-exhaustion.
//   Returns Split { new_loc: <half with anchor>, relocs:[A Left{0,0}, B Left{0,0}] }.
...
//     new = (old<<3) - 80  -> Transform::Left{3,-80}.
//   Returns Split { new_loc: pluripotent,
//                   relocs:[carve Transform, remainder Transform::Left{0,0}] }.
...
//   pluripotent (Transform per split_off_pluripotent). Mirror of split_off but the
//   carve is the middle and both tails stay linked.
//   Returns Split { new_loc: mid,
//                   relocs:[left Left{0,0}, mid Transform, right Left{0,0}] }.
...
//   vptr: uniform readdress  -> Transform (Left if new_shift>=old_shift, else Right).
...
enum ArenaInsertDelta {
    ...
    Readdressed { block_id: usize, new_virt: isize, transform: Transform },// whole-block readdress
    Split { new_loc: (usize, isize), chunks: [Reloc; 3], n_chunks: u8 },    // partitioned (<=3; split_mid = left/mid/right)
    Shoved {
...
//
// Consumer surface: ArenaInsertDelta/Reloc are arena-internal. `execute`
// consumes them to drive the walker (apply_relocs rewrites in-arena ptrs in
// place via unsafe disjoint borrows) on the walker/revptr paths — no RemapSet
// returned. On the pointerless path it condenses them to the consumer-facing
// RemapSet (bounded <=3, fixed enum — hence Split carries [Reloc; 3]+n_chunks,
// not a Vec). The consumer never sees Transform or phys.
//
