...
fn test_inode_find_child_basic() {
    ...
    assert_eq!(inode_find_child(buf.as_ptr(), symbols_offset, 6, 10), Some(0));
    assert_eq!(inode_find_child(buf.as_ptr(), symbols_offset, 6, 30), Some(2));
    assert_eq!(inode_find_child(buf.as_ptr(), symbols_offset, 6, 60), Some(5));
    assert_eq!(inode_find_child(buf.as_ptr(), symbols_offset, 6, 15), None);
    assert_eq!(inode_find_child(buf.as_ptr(), symbols_offset, 6, 5), None);
    assert_eq!(inode_find_child(buf.as_ptr(), symbols_offset, 6, 65), None);
}
#[test]
fn test_inode_find_child_min_count() {
    let symbols_offset = 2;
    let buf = make_inode_buf(symbols_offset, &[100, 200]);
    assert_eq!(inode_find_child(buf.as_ptr(), symbols_offset, 2, 100), Some(0));
    assert_eq!(inode_find_child(buf.as_ptr(), symbols_offset, 2, 200), Some(1));
    assert_eq!(inode_find_child(buf.as_ptr(), symbols_offset, 2, 150), None);
}
#[test]
fn test_inode_find_child_boundary_bytes() {
    ...
    for &byte in &[0x00, 0x7F, 0x80, 0xFF] {
        ...
        assert_eq!(inode_find_child(buf.as_ptr(), symbols_offset, 2, byte), Some(0));
    }
}
#[test]
fn test_inode_find_child_lower_bound_basic() {
    ...
    let symbols = [10u8, 20, 30, 40, 50, 60];
    let buf = make_inode_buf(symbols_offset, &symbols);
    assert_eq!(inode_find_child_lower_bound(buf.as_ptr(), symbols_offset, 6, 10), 0);
    assert_eq!(inode_find_child_lower_bound(buf.as_ptr(), symbols_offset, 6, 30), 2);
    assert_eq!(inode_find_child_lower_bound(buf.as_ptr(), symbols_offset, 6, 60), 5);
    assert_eq!(inode_find_child_lower_bound(buf.as_ptr(), symbols_offset, 6, 15), 1);
    assert_eq!(inode_find_child_lower_bound(buf.as_ptr(), symbols_offset, 6, 5), 0);
    assert_eq!(inode_find_child_lower_bound(buf.as_ptr(), symbols_offset, 6, 65), 6);
}
#[test]
fn test_inode_find_child_lower_bound_boundary_bytes() {
    let symbols_offset = 2;
    let buf = make_inode_buf(symbols_offset, &[0x00, 0x7F, 0x80, 0xFF]);
    assert_eq!(inode_find_child_lower_bound(buf.as_ptr(), symbols_offset, 4, 0x00), 0);
    assert_eq!(inode_find_child_lower_bound(buf.as_ptr(), symbols_offset, 4, 0x7F), 1);
    assert_eq!(inode_find_child_lower_bound(buf.as_ptr(), symbols_offset, 4, 0x80), 2);
    assert_eq!(inode_find_child_lower_bound(buf.as_ptr(), symbols_offset, 4, 0xFF), 3);
    assert_eq!(inode_find_child_lower_bound(buf.as_ptr(), symbols_offset, 4, 0x40), 1);
    assert_eq!(inode_find_child_lower_bound(buf.as_ptr(), symbols_offset, 4, 0x90), 3);
}
#[test]
