...
impl<PTR: TrieIndex> FixedLenNode<PTR> {
    ...
    fn set_internal_child(&mut self, nib: usize, arena_idx: PTR) {
        debug_assert!(nib < 16);
        debug_assert!(
            arena_idx != PTR::max_value_sentinel(),
            "arena index max_value is sentinel"
        );
        self.clear_leaf(nib);
...
impl<T, PTR: TrieIndex> FixedLenNibbleTrie<T, PTR> {
        ...
        loop {
            ...
            match simd_find_divergence::<8>(&key, ref_key, confirmed) {
                ...
                DivergeResult::At(diverge) if diverge < prefix_len => {
                    ...
                    let old_node =
                        std::mem::replace(&mut self.arena[node_idx.as_usize()], new_parent);
                    ...
                    if node.is_leaf(nib) {
                        ...
                        match simd_find_divergence::<8>(&key, existing_key, confirmed) {
                            ...
                            DivergeResult::At(d) => {
                                ...
                                self.arena[node_idx.as_usize()]
                                    .set_internal_child(nib, split_idx);
                                self.sort_internal_children(node_idx);
                                return Ok(new_ki.as_usize());
                            }
                        }
    ...
    fn walk_optimize(
        ...
        if node.is_terminal() {
            ...
            unsafe {
                std::ptr::write(
                    new_values.as_mut_ptr().add(new_ki),
                    std::ptr::read(self.values.as_ptr().add(old_ki)),
                );
            }
