...
impl Ord for KeyRef {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        match (self, other) {
            ...
            (KeyRef::Inline(a), KeyRef::Inline(b)) => {
                cmp_slice_scalar(a.as_slice(), b.as_slice())
            }
...
impl<T: FixedLenKey> TreeKey for T {
    type Stored = T;
    type Needle = T;
    fn into_stored(self, _buf: &mut Vec<u8>) -> T {
        self
    }
    fn as_needle(&self) -> &T {
        self
...
impl<K: FixedLenKey> StoredKey for K {
    type Needle = K;
    fn cmp_key(stored: &K, needle: &K, _buf: &[u8]) -> Ordering {
        stored.cmp(needle)
    }
    fn eq_key(stored: &K, needle: &K, _buf: &[u8]) -> bool {
        stored == needle
    }
    fn cmp_stored(a: &K, b: &K, _buf: &[u8]) -> Ordering {
        a.cmp(b)
    }
}
    ...
    [(); NP1]:,
{
    ...
    fn new() -> Self {
        Self { keys: TinyArray::new(), ptrs: [None; NP1] }
    }
    ...
    fn insert_leaf(
        &mut self,
        needle: &<K as TreeKey>::Needle,
        stored: <K as TreeKey>::Stored,
        buf: &[u8],
    ) -> usize {
...
// ---------------------------------------------------------------------------
#[allow(dead_code)]
impl<K, V, PTR, const N: usize, const NP1: usize> CTree<K, V, PTR, N, NP1>
where
    K: TreeKey + SearchStrategy,
    K::Stored: StoredKey,
    PTR: TrieIndex,
    ...
    [(); NP1]:,
{
    ...
    fn relocate(&mut self, gapful: bool) -> Vec<usize> {
        ...
        debug_assert_eq!(
            live, self.n_leaves,
            "relocate: live count {live} != n_leaves {}",
            self.n_leaves
        );
    ...
    fn split_inode(
        ...
        let goes_right =
            StoredKey::cmp_stored(&new_stored, &mid_stored, &self.key_buf) != Ordering::Less;
        if goes_right {
            ...
            let pos = self
                .find_position_for_stored_in_keys(&new_stored, &self.inodes[parent_idx].keys);
    ...
    fn find_position_for_stored_in_keys(
        &self,
        stored: &K::Stored,
        keys: &TinyArray<K::Stored, N>,
    ) -> usize {
    ...
    pub fn get_cursor(&self) -> Cursor<'_, K, V, PTR, N, NP1> {
        ...
        Cursor { tree: self, leaf_idx, position: 0 }
    }
    pub fn get_cursor_mut(&mut self) -> CursorMut<'_, K, V, PTR, N, NP1> {
        let leaf_idx = self.first_leaf();
        CursorMut { tree: self, leaf_idx, position: 0 }
    }
    pub fn cursor_at(&self, key: &<K as TreeKey>::Needle) -> Cursor<'_, K, V, PTR, N, NP1> {
        let (leaf_idx, pos) = self.locate(key);
        Cursor { tree: self, leaf_idx, position: pos }
    }
}
// ---------------------------------------------------------------------------
// Cursor impl
// ---------------------------------------------------------------------------
impl<'a, K, V, PTR, const N: usize, const NP1: usize> Cursor<'a, K, V, PTR, N, NP1>
where
    K: TreeKey + SearchStrategy,
    K::Stored: StoredKey,
    PTR: TrieIndex,
...
impl<'a, K, V, PTR, const N: usize, const NP1: usize> CursorMut<'a, K, V, PTR, N, NP1>
where
    K: TreeKey + SearchStrategy,
    K::Stored: StoredKey,
    PTR: TrieIndex,
