...
pub fn generate_keys_nonzero(
    ...
    generate_keys(mode, n, corpus).into_iter().filter_map(NonZeroBytes::new).collect()
}
...
pub fn resolve_sizes(arg: Option<&str>) -> Vec<usize> {
    ...
    if let Some((lo_s, hi_s)) = arg.split_once("..") {
        ...
        let filtered: Vec<usize> =
            all_sizes.iter().filter(|&&s| s >= lo && s <= hi).copied().collect();
        if filtered.is_empty() {
            eprintln!(
                "Error: no canonical sizes in range {lo}..{hi}. Available: {:?}",
                all_sizes
            );
            std::process::exit(1);
        }
    ...
    let requested: Vec<usize> =
        arg.split(',').filter_map(|s| s.trim().parse::<usize>().ok()).collect();
    let filtered: Vec<usize> =
        requested.iter().filter(|s| all_sizes.contains(s)).copied().collect();
    let rejected: Vec<usize> =
        requested.iter().filter(|s| !all_sizes.contains(s)).copied().collect();
    if !rejected.is_empty() {
        eprintln!(
            "Warning: sizes {rejected:?} not in canonical sizes {:?}, ignoring",
            all_sizes
        );
    }
    if filtered.is_empty() {
