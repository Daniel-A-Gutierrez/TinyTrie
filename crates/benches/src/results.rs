use super::keygen::{COL, KeyMode, NAME_COL};
...
pub fn fmt_bytes_per(bytes: f64) -> String {
    if bytes >= 1e3 { format!("{:.0}", bytes) } else { format!("{:.1}", bytes) }
}
// ── Result storage ──────────────────────────────────────────────────
pub type ResultMap = HashMap<String, Vec<f64>>;
fn fmt_table(
    title: &str,
    unit: &str,
    data: &ResultMap,
    ...
    let active_names: Vec<&str> =
        names.iter().filter(|n| data.contains_key(*n as &str)).copied().collect();
    ...
    for &sz in sizes {
        s.push_str(&format!("{:>COL$}", sz));
    }
    s.push('\n');
    for name in &sorted {
        s.push_str(&format!("{:<NAME_COL$}", name));
        for &val in data.get(**name as &str).unwrap() {
            s.push_str(&format!("{:>COL$}", fmt_val(val)));
        }
        s.push('\n');
    }
    s
}
pub fn print_table(title: &str, unit: &str, data: &ResultMap, sizes: &[usize], names: &[&str]) {
    let s = fmt_table(title, unit, data, sizes, names, fmt_rate, true);
    if !s.is_empty() {
        print!("{s}");
    }
}
pub fn print_mem_table(data: &ResultMap, sizes: &[usize], names: &[&str]) {
...
pub fn load_results(json_path: &str) -> ResultsFile {
    match std::fs::read_to_string(json_path) {
        Ok(s) => serde_json::from_str(&s).unwrap_or_default(),
        Err(_) => ResultsFile::default(),
    }
}
pub fn save_results(data: &ResultsFile, json_path: &str, md_path: &str) {
    ...
    for (section, rows) in &data.sections {
        ...
        if is_mem {
            entries.sort_by(|a, b| {
                a.1.first().partial_cmp(&b.1.first()).unwrap_or(std::cmp::Ordering::Equal)
            });
        } else {
            entries.sort_by(|a, b| {
                b.1.first().partial_cmp(&a.1.first()).unwrap_or(std::cmp::Ordering::Equal)
            });
        }
        md.push_str(&format!("\n─── {section} ───\n"));
        md.push_str(&format!("{:<NAME_COL$}", ""));
        for &sz in &all_sizes {
            md.push_str(&format!("{:>COL$}", sz));
        }
        md.push('\n');
