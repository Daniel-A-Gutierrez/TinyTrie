...
fn node_size_default() {
    // Default PTR_T=u32, LEN_T=u16 should still be 80 bytes
    assert_eq!(std::mem::size_of::<Node<u32, u16>>(), 80);
}

#[test]
fn node_size_compact() {
    // Compact PTR_T=u16, LEN_T=u16 should be 48 bytes (fits cache line)
    assert_eq!(std::mem::size_of::<Node<u16, u16>>(), 48);
}
...
fn leaf_and_offset_set_on_creation() {
    ...
    assert_ne!(root.leaf, u32::zero(), "root leaf field should be set");
    assert_ne!(root.raw_offset(), 0, "root offset field should be set");
    // offset should point to the key in buf
    let (off, len) = trie.index[root.leaf.as_usize()];
    assert_eq!(root.raw_offset(), off as u64, "offset should match index offset");
...
// ── Terminal bit packing tests ─────────────────────────────────────

#[test]
fn terminal_bit_packing() {
    let mut node: Node<u32, u16> = Node::new();
    assert!(!node.is_terminal());
    assert_eq!(node.raw_offset(), 0);

    node.set_terminal(true);
    assert!(node.is_terminal());
    assert_eq!(node.raw_offset(), 0); // terminal bit doesn't affect offset

    node.set_raw_offset(0x1234_5678);
    assert!(node.is_terminal()); // terminal bit preserved
    assert_eq!(node.raw_offset(), 0x1234_5678);

    node.set_terminal(false);
    assert!(!node.is_terminal());
    assert_eq!(node.raw_offset(), 0x1234_5678); // offset preserved

    // Large offset should not interfere with terminal bit
    node.set_raw_offset(0x7FFF_FFFF_FFFF_FFFF); // max valid offset (bit 62 set)
    node.set_terminal(true);
    assert!(node.is_terminal());
    assert_eq!(node.raw_offset(), 0x7FFF_FFFF_FFFF_FFFF);
}
...
#[test]
fn node_size_u8() {
    // Compact PTR=u8, LEN=u16 should be 32 bytes
    assert_eq!(std::mem::size_of::<Node<u8, u16>>(), 32);
}
...
fn u8_near_capacity() {
    let mut trie: NibbleTrie<i32, u8, u16> = NibbleTrie::new();
    // u8 max_value = 255, near_capacity when arena.len() >= 255 or index.len() >= 255
...
fn u8_overflow() {
    let mut trie: NibbleTrie<i32, u8, u16> = NibbleTrie::new();
    // With u8 PTR, index.len() > 255 triggers overflow.
    // Insert 255 keys (index.len() = 256 including dummy).
    // The 255th insert should succeed (index 255 = u8::MAX is valid).
    // The 256th insert should fail (index.len() = 256 > 255).
    for i in 0..255u32 {
